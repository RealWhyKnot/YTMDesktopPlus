...
log.errorHandler.startCatching({
  ...
  onError({ error, processType, versions }) {
    ...
    if (!app.isReady()) {
      dialog.showErrorBox(`YTMDesktopPlus Crashed`, `Application crashed before ready\n\n${dialogMessage}`);
    } else {
      ...
      result = dialog.showMessageBoxSync({
        title: "Error",
        message: "YTMDesktopPlus Crashed",
      ...
      if (result === 0 || result === 2) {
        clipboard.writeText(`YTMDesktopPlus Crashed\n\n${dialogMessage}`);
      }
    }
...
const template: MenuItemConstructorOptions[] = [{ role: "appMenu", label: "YTMDesktopPlus" }, { role: "editMenu" }];
...
const createYTMView = (): void => {
  ...
  ytmView.webContents.on("page-title-updated", (_event, title) => {
    if (mainWindow) {
      mainWindow.setTitle(`${title} | YTMDesktopPlus`);
    }
  });
