...
import { initializeTestSeams, isTestRun, takeBrokenHookStage } from "./test-seams";
...
const createYTMView = (): void => {
  ...
  const brokenHookStage = takeBrokenHookStage();
  ytmView = new BrowserView({
    webPreferences: {
      ...
      additionalArguments: brokenHookStage ? [`--ytmd-test-break-hooks=${brokenHookStage}`] : []
    }
  });
  companionServer.provide(store, memoryStore, ytmView);
  customCss.provide(store, ytmView);
