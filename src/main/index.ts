...
let roomWindow: BrowserWindow = null;
...
function openRoomFromLink(roomId: string) {
  if (!store.get("integrations").listenAlongRoomsEnabled) return;
  memoryStore.set("listenAlongRoomJoinPrompt", roomId);
...
// One relay room at a time, hosted or joined. Opens a connection only when the
// user starts or joins a room; the master toggle gates every entry point.
...
if (store.get("integrations").listenAlongRoomsEnabled === undefined) {
...
store.onDidAnyChange(async (newState, oldState) => {
  ...
  // The master toggle closes any open room outright: off means no connection
  // to the room service at all.
  if (oldState.integrations.listenAlongRoomsEnabled && newState.integrations.listenAlongRoomsEnabled === false) {
    roomSession.leave();
...
const createOrShowRoomWindow = (): void => {
  if (roomWindow !== null) {
    roomWindow.focus();
    return;
  }
  ...
  roomWindow = new BrowserWindow({
    width: 400,
    height: 600,
    x: anchorBounds ? Math.round(anchorBounds.x + (anchorBounds.width / 2 - 200)) : undefined,
    y: anchorBounds ? Math.round(anchorBounds.y + (anchorBounds.height / 2 - 300)) : undefined,
    minimizable: false,
    maximizable: false,
    resizable: false,
    frame: false,
    ...
    webPreferences: {
      sandbox: true,
      contextIsolation: true,
      preload: path.join(__dirname, `../renderer/windows/room/preload.js`),
      devTools: store.get("developer.enableDevTools")
    }
  });

  roomWindow.once("closed", () => {
    roomWindow = null;
  });

  roomWindow.webContents.setWindowOpenHandler(() => ({ action: "deny" }));

  roomWindow.webContents.on("will-navigate", event => {
    if (process.env.NODE_ENV === "development") if (event.url.startsWith("http://localhost")) return;

    event.preventDefault();
  });

  roomWindow.on("ready-to-show", () => {
    roomWindow.show();
    if (process.env.NODE_ENV === "development") {
      roomWindow.webContents.openDevTools({ mode: "detach" });
    }
  });

  if (ALL_WINDOWS_VITE_DEV_SERVER_URL) roomWindow.loadURL(ALL_WINDOWS_VITE_DEV_SERVER_URL + "/windows/room/index.html");
  else roomWindow.loadFile(path.join(__dirname, `../renderer/windows/room/index.html`));
};
...
app.on("ready", async () => {
  ...
  // Handle room ipc. Every entry point rechecks the master toggle so a stale
  // window cannot open a connection after the feature was turned off.
  const isRoomSender = (sender: Electron.WebContents) => roomWindow && sender === roomWindow.webContents;
  const roomsEnabled = () => store.get("integrations").listenAlongRoomsEnabled;

  ipcMain.on("room:openWindow", event => {
  ...
  ipcMain.on("room:host", (event, displayName: string) => {
    if (!isRoomSender(event.sender) || !roomsEnabled()) return;
  ...
  ipcMain.on("room:join", (event, roomId: string, displayName: string) => {
    if (!isRoomSender(event.sender) || !roomsEnabled()) return;
    const name = sanitizeDisplayName(displayName);
    if (!name || !isRoomId(roomId)) return;
  ...
  ipcMain.on("room:grant", (event, memberId: string, role: number) => {
    if (!isRoomSender(event.sender)) return;
    if (typeof memberId !== "string" || (role !== 0 && role !== 1)) return;

    roomSession.grant(memberId, role as RoomRole);
  });

  ipcMain.on("room:control", (event, action: string, value?: unknown) => {
    if (!isRoomSender(event.sender)) return;
    if (!CONTROL_ACTIONS.includes(action as ControlAction)) return;

    roomSession.control(action as ControlAction, value as number | string | undefined);
  });

  ipcMain.on("room:resume", event => {
    if (!isRoomSender(event.sender)) return;

    roomSession.resume();
  });

  ipcMain.on("roomWindow:close", event => {
    if (!isRoomSender(event.sender)) return;
  ...
  trayContextMenu = Menu.buildFromTemplate([
    ...
    },
    {
      ...
      type: "normal",
      click: () => {
        if (store.get("integrations").listenAlongRoomsEnabled) createOrShowRoomWindow();
      }
    },
    {
      type: "separator"
